...
class MenuController: NSObject, NSMenuDelegate {
    ...
    @IBOutlet weak var netData: NSMenuItem!
...
//            if URL(string: uri) == nil {
    ...
    @IBAction func getNetData(_ sender: Any) {
        let dataURL = "https://api.64clouds.com/v1/getServiceInfo?veid=1164687&api_key=private_JYPrGz7YawpI4roaONin7Hc1"

        Alamofire.request(dataURL).responseJSON {
            response in
            if response.result.isSuccess {
                if let result = response.result.value {
                    let JSON = result as! NSDictionary

                    let data_counter: Int64 = JSON["data_counter"] as! Int64
                    let plan_monthly_data: Int64 = JSON["plan_monthly_data"] as! Int64

                    print(data_counter);
                    
                    self.netData.title = self.convertFileSize(data_counter)+"/"+self.convertFileSize(plan_monthly_data)
                }
            } else {
    ...
    func convertFileSize(_ size: Int64) -> String {
        ...
        let mb = kb * 1024
        let gb = mb * 1024
        return String(format: "%.2fGB", Float(size) / Float(gb))
    }
